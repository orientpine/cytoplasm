...
| `thread_pointer.py` | 승인 요청 CLI 가 게시 직후 내는 `APPROVAL-THREAD <키>=<값> url=<링크>` 줄의 단일 정의. 좌표는 레코드의 `approval_guild_id`·`approval_thread_id`, 링크는 `owner_message.discord_link`, 좌표 결손은 `url=unavailable search=<값>`(추측 금지). I/O 없음 |
...
- **승인 표면 해석**: 표면은 `approval_surface.py`(정책, I/O 없음)가 정하고 `approval_directory.py`(유일한 해석기)가 실제 채널로 바꾼다. 승인 producer가 스스로 채널을 해석하면 `tests/unit/test_approval_lifecycle_conformance.py`가 빌드를 깨뜨린다 — 산문이 아니라 코드가 강제한다.
  ...
  - **현재 라우팅 (AS-SSOT, v8 + 요청별 스레드 — 2026-09-01 소유자 지시)**: calendar·coordination·wiki·mail compose·mail reply·budget·patent-export·obsidian-write·todo·repair(v8, ON-4 — Ops 봇 초대 전제) = **개인 서버 `#agent-chat`의 요청별 스레드**(`resolve_new_binding(request=RequestThread)` → `agent_chat_request_thread`: 지시 메시지가 이 채널 소속이면 앵커(400=message id 재사용), 아니면 안내 메시지를 먼저 게시하고 그 메시지에 앵커, 스레드 이름은 `<라벨> · <제목≤40>` — 조회 없음, 요청끼리 공유 없음; record.channel_id=스레드 id라 기존 리액션 워처·리마인더는 무변경 동작). 결과 통지는 레코드 `approval_thread_id` 로 같은 스레드에 게시하고 `origin_notice.ThreadOutcome` 으로 종결 표시(접두어+아카이브). kind별 `승인-<kind>` 스레드(v7, `agent_chat_thread` find-or-create)는 `request=None` 레거시 호출에만 남으며 범위 내 호출부는 `tests/unit/test_request_thread_adoption_conformance.py` 가 `request=` 를 강제한다. skill-deploy·peer-attest·skill-publish·personal-skill-submit·managed 활성화 = **개인 서버 `#approvals`**(2차 주체인 peer 봇 또는 그룹 관리자가 같은 채널을 봐야 함). v6 이하로 게시된 기존 DM pending 레코드는 저장 바인딩(S2)대로 DM에서 종결된다. **주의: `#approvals`에는 메일·과제비·캘린더·위키·수리 승인이 단 한 건도 게시된 적이 없다**(전 생애 117건 실측).
