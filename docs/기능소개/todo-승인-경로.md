...
> **이 문서는 TRACK-A(PR #125) 착지 시점의 기록이다.** 그 뒤 두 가지가 바뀌었다.
> ① 승인 표면은 오너 DM이 아니라 `#agent-chat` 의 `승인-todo` 스레드다(정책 v7).
> ② ✅ 이후의 등록은 사람이 `create` 를 다시 부르는 것이 아니라 워처가 수행한다.
> 현행 계약은 [todo 승인 후 자동 등록](todo-승인-후-자동-등록.md) 이 소유한다.
...
- 재사용: `automation/interop/approval_lifecycle.py`(파사드), `approval_lease.py`(FileKeyLease + PostingJournal), `approval_surface.py`(`ApprovalKind.TODO` — 정책 0에서 오너 DM, 정책 7부터 `agent-chat` 스레드), `approval_directory.py`
