"""Deploy-local deterministic sensitivity gate for document-type handling."""
...
class TagRule:
    """One sensitivity tag with deterministic literal and regex triggers."""
...
class SensitivityVerdict:
    """The pre-LLM decision recorded without retaining the inspected text."""
...
class RuleLoadError(RuntimeError):
    """The bundled sensitivity policy cannot be parsed safely."""
...
def load_rules(path: Path) -> tuple[TagRule, ...]:
    """Read YAML when available, otherwise parse the constrained bundled shape."""
    ...
    for tag, spec in tags.items():
        if not isinstance(tag, str) or not isinstance(spec, dict):
            raise RuleLoadError("invalid sensitivity rule")
...
def evaluate(text: str, rules: tuple[TagRule, ...]) -> SensitivityVerdict:
    """Return the fail-closed patent routing verdict before any LLM call."""
    lowered = text.lower()
    tags = tuple(
        rule.tag
        for rule in rules
        if any(keyword.lower() in lowered for keyword in rule.keywords)
        or any(re.search(pattern, text) for pattern in rule.patterns)
    )
    return SensitivityVerdict(sensitive="patent-sensitive" in tags, tags=tags)
