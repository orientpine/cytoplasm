...
def refusal(script: Path, *, env: Mapping[str, str] | None = None) -> str | None:
    ...
    raw_root = environment.get(LIVE_ROOT_ENV, "").strip()
    root = Path(raw_root).expanduser() if raw_root else GOVERNED_LIVE_ROOT
    try:
        repo = _repo_root()
