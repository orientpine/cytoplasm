...
def test_split_filename_and_size():
    ...
    assert split_filename_and_size("KIMM_AIDX_v03.mp4 547980148") == (
        "KIMM_AIDX_v03.mp4", 547980148,
    )
...
def test_parse_view_async_strips_size_from_attachment_name():
    ...
    <div id="mvw_wrap">
      ...
      <a href="/mail/download?attachId=1">KIMM_AIDX_v03.mp4 547980148</a>
      <a href="/mail/download?attachId=2">small.pdf 12345</a>
    </div>
    """
    out = parse_view_async_html(html)
    assert len(out["attachments"]) == 2
    assert out["attachments"][0]["filename"] == "KIMM_AIDX_v03.mp4"
...
def test_writer_exact_output(tmp_path):
    from mailon.writer import Mail, Attachment, write_mail
...
def test_scraper_mail_list_parse():
    ...
    page1_resp = json.dumps({
        "result": True,
        "folder": {"folderUid": 49527, "newMsgNum": 2},
        "contents": [
            {"mailUid": 100, "folderUid": 49527, "subject": "Hello",
             "adrFrom": '"A" <a@b.com>', "adrTo": '<me@x.com>',
             "timeMillis": 1776836838000, "isSeen": 0, "isFlagged": 0,
             "msgSize": 1000, "attachCount": 0},
            {"mailUid": 101, "folderUid": 49527, "subject": "Re: World",
    ...
    empty_resp = json.dumps({
        "result": True, "folder": {"folderUid": 49527}, "contents": []
    })
    ...
    scraper.folder_uid = "49527"  # manually set since we skip resolve_inbox_folder_uid()
    ...
    assert refs[0].folder_uid == "49527"
...
def test_scraper_read_mail_parse():
    ...
    <div>
      ...
      <input type="hidden" id="folderUid" value="49527" />
    ...
    ref = MailRef(
        uid="777", folder_uid="49527", subject="fallback-subj",
...
class FakeComposeBrowser:
    ...
    def upload(self, selector, paths):
        self.upload_calls.append((selector, paths))
...
def test_send_dry_run_reports_attachment_without_uploading(tmp_path):
    from mailon.send import ComposeSender, SendRequest
    ...
    browser = FakeComposeBrowser()
    request = SendRequest(
        recipients=("recipient@example.test",), cc=(),
        subject="offline subject", body="offline body",
...
class VerifyFakeBrowser(FakeComposeBrowser):
    ...
    def __init__(self, *, row_uid=SENT_FOLDER_UID, send_call_result="default",
        ...
        self.form_dumps = ([{"method": "send", "to": f"<{VERIFY_RECIPIENT}>",
                             "from": "me@example.invalid", "content": "verify body"}]
    ...
    def eval_json(self, script):
        names = [path.name for _selector, paths in self.upload_calls for path in paths]
        if "ready: false" in script and "queued" in script:
            return {
                "ready": True,
                "queued": len(names),
                "records": len(names),
        ...
        if "uid_count" in script and "record_count" in script:
            return {
                "represented": names,
                "uid_count": len(names),
                "record_count": len(names),
            }
        return super().eval_json(script)
...
def test_missing_attachment_is_classified_as_input_validation(tmp_path):
    import pytest

    from mailon.send import AttachmentValidationError, SendRequest

    with pytest.raises(AttachmentValidationError) as caught:
        SendRequest(
            recipients=(VERIFY_RECIPIENT,), cc=(), subject=VERIFY_SUBJECT,
            body="verify body", attachments=(tmp_path / "missing.txt",),
        )

    assert caught.value.error_code == "attachment_invalid"
    assert caught.value.stage == "validation"
    assert caught.value.retryable is False


def test_existing_attachment_uploads_with_structured_verified_result(tmp_path):
    import json

    from mailon.send import SendRequest

    attachment = tmp_path / "fixture.txt"
    attachment.write_text("offline fixture", encoding="utf-8")
    ...
    request = SendRequest(
        recipients=(VERIFY_RECIPIENT,), cc=(), subject=VERIFY_SUBJECT,
        body="verify body", attachments=(attachment,),
    )

    result = _verify_sender(browser).send(request, dry_run=False)
    payload = json.loads(result.to_json())

    assert browser.upload_calls == [("#uploaderAttach", (attachment,))]
    assert payload["status"] == "submitted"
    assert payload["verified"] is True
    assert payload["attachment_count"] == 1
    assert len(payload["attachment_manifest_sha256"]) == 64

def test_send_error_payload_is_stable_and_safe():
    from mailon.main import _send_error_payload
    from mailon.send import AttachmentUnsupportedError

    payload = _send_error_payload(AttachmentUnsupportedError("private implementation detail"))

    assert payload["status"] == "error"
    assert payload["error_code"] == "attachment_unsupported"
    assert payload["stage"] == "attachment_upload"
    assert payload["retryable"] is False
    assert "private implementation detail" not in json.dumps(payload, ensure_ascii=False)
...
def test_send_with_attachment_uploads_and_reports_count(tmp_path):
    from mailon.send import SendRequest
    ...
    assert browser.upload_calls == [("#uploaderAttach", (attachment,))]
    assert any("window.uploader.upload()" in script for script in browser.javascript_calls)
    assert result.attachment_count == 1
    ...
    browser = VerifyFakeBrowser(
        form_dumps=[
            {"method": "send", "to": f"<{VERIFY_RECIPIENT}>",
             "from": "me@example.invalid", "content": "undefined"},
            {"method": "send", "to": f"<{VERIFY_RECIPIENT}>",
             "from": "me@example.invalid", "content": "verify body"},
        ],
        list_responses=[NO_MATCH_PAGE, MATCH_PAGE],
...
def test_send_fails_closed_on_tome_redirect_mismatch():
    ...
    browser = VerifyFakeBrowser(
        form_dumps=[{"method": "tome", "to": f"<{VERIFY_RECIPIENT}>",
                     "from": "someone-else@example.invalid", "content": "verify body"}],
        list_responses=[NO_MATCH_PAGE],
        network_listings=[BASELINE_LISTING],
    )
...
def test_resolve_current_folder_waits_for_late_row_render():
    ...
    browser = VerifyFakeBrowser(row_uid=[None, None, "49527"])
...
INBOX_UID = "49527"
ACCOUNT = "orientpine@example.invalid"
...
def test_send_tokenizes_each_recipient_individually():
    ...
    multi_match = {"result": True, "contents": [{
        ...
        "adrTo": "<a@example.invalid>, <b@example.invalid>, <c@example.invalid>",
    ...
    browser.form_dumps = [{
        ...
        "to": "a@example.invalid,b@example.invalid,c@example.invalid",
    ...
    assert "a@example.invalid" in tokenize_calls[0]
    assert "c@example.invalid" in tokenize_calls[0]
...
def test_mail_match_accepts_first_recipient_only_adrto():
    ...
    item = {"subject": VERIFY_SUBJECT,
            "adrTo": "<a@example.invalid>",  # first recipient only
            "timeMillis": NOW_MS - 5_000}
...
def test_resolve_inbox_folder_uid_waits_for_late_rows():
    ...
    browser = MagicMock()
    browser.eval_js.side_effect = ["null", "null", json.dumps("49527")]
...
def test_parse_candidates_history_cell_with_org_suffix():
    ...
    assert out[0].org == "<example-organization>"
...
def test_parse_candidates_ignores_placeholder_and_patternless_cells():
    ...
    cells = [
        ...
        "AX융합연구센터",              # no "name" <email> pattern
