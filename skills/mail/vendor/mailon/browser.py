...
class AgentBrowser:
    ...
    def upload(self, ref: str, paths: tuple[Path, ...]) -> None:
        """Attach local files to a file input using agent-browser upload."""
        if not paths:
            return
        resolved = [str(path.resolve()) for path in paths]
        self._run(["upload", ref, *resolved], timeout_override=max(self.timeout, 180))
