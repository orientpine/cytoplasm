"""Deterministic pre-LLM sensitivity gate for mail triage (W4-2, constraint 6).

Vendored from the W2-3 meeting skill gate (skills/meeting/scripts/meeting_gate.py)
so the mail skill stays self-contained when deployed standalone. The RULES file
stays single-source: skills/mail/configs/sensitivity-rules.yaml is a synced copy
of configs/sensitivity-rules.yaml (same per-skill-copy pattern as meeting).

The gate runs on subject+sender+FULL BODY immediately after the W4-1 read,
BEFORE any LLM routing decision. No LLM participates in this decision.
"""
...
def _parse_rules_fallback(raw: str) -> dict:
    """Minimal indentation parser for the fixed rules schema (no PyYAML)."""
