...
이 명령은 변경 요약과 `sha256`을 포함한 cha DM을 게시하고, **✅를 먼저,
...
이 인자는 **결과 통지 경로만** 바꾼다. 확인(✅/⛔)은 종전대로 cha와의 확인 메시지에서만
이뤄지고, 초안 해시에도 들어가지 않는다(같은 변경이면 해시도 같다). 실행·취소·만료가
확정되면 감시자가 그 지시 메시지의 스레드(`캘린더 확정 (draft <id>)`)에 결과를 올린다.
