"""speechtotext 변경 명령을 관리자 배포본으로 한정하는 지연 게이트."""
...
def refusal(script: Path, *, env: Mapping[str, str] | None = None) -> str | None:
    ...
    root_value = environment.get(LIVE_ROOT_ENV, "").strip()
    root = Path(root_value).expanduser() if root_value else GOVERNED_LIVE_ROOT
    try:
        repo = _repo_root()
