...
def decode(raw: str) -> TodoApprovalRecord:
    try:
        ...
        return TodoApprovalRecord(
            ...
            approval_guild_id=_optional_string(payload, "approval_guild_id"),
            render_version=_optional_string(payload, "render_version"),
            tasklist=tasklist,
            title=title,
            notes=notes,
            due=due,
...
def _payload(record: TodoApprovalRecord) -> dict[str, JsonValue]:
    return {
        ...
        **({"render_version": record.render_version} if record.render_version is not None else {}),
