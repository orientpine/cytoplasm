"""doctype CLI의 배포 사본 실행 차단을 고정한다."""
from __future__ import annotations

import os
import shutil
import subprocess
import sys
from pathlib import Path

from automation import skill_mount

ROOT = Path(__file__).parents[2]
CLI = ROOT / "skills/doctype/scripts/doctype_cli.py"
SCRIPTS = ROOT / "skills/doctype/scripts"


def _run(cli: Path, live_root: Path, *args: str) -> subprocess.CompletedProcess[str]:
    env = {
        **os.environ,
        "AUTOPHAGY_REPO_ROOT": str(ROOT),
        "AUTOPHAGY_SKILL_LIVE_ROOT": str(live_root),
    }
    return subprocess.run(
        [sys.executable, str(cli), *args],
        cwd=ROOT,
        env=env,
        capture_output=True,
        text=True,
        check=False,
    )


def test_mutating_stale_copy_is_refused(tmp_path: Path) -> None:
    stale = tmp_path / "stale" / "scripts"
    stale.mkdir(parents=True)
    for source in SCRIPTS.iterdir():
        if source.is_file():
            shutil.copy2(source, stale / source.name)
    live = tmp_path / "live"
    (live / "doctype" / "scripts").mkdir(parents=True)

    result = _run(stale / "doctype_cli.py", live, "register-from-example", "--name", "x", "--example", "missing.txt")

    assert result.returncode == 3
    assert skill_mount.STALE_COPY_MARKER in result.stderr


def test_guard_does_not_fire_without_live_doctype(tmp_path: Path) -> None:
    stale = tmp_path / "stale" / "scripts"
    stale.mkdir(parents=True)
    for source in SCRIPTS.iterdir():
        if source.is_file():
            shutil.copy2(source, stale / source.name)

    result = _run(stale / "doctype_cli.py", tmp_path / "live", "register-from-example", "--name", "x", "--example", "missing.txt")

    assert skill_mount.STALE_COPY_MARKER not in result.stderr
