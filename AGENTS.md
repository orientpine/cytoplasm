...
| `stt_window_run` | module | `skills/speechtotext/scripts/stt_window_run.py` | **구간당 whisper.cpp 프로세스 하나** — 정규화된 wav 를 `-ot <ms> -d <ms>` 로 구간만 읽고(슬라이스 파일 0, whisper 가 절대 오프셋을 보고하므로 시간축 보정 불필요), 예산도 구간 단위다. `accept()` 가 payload 를 판정해 `invalid-json`·`no-transcription`·`repetition=…` 사유를 돌려주고 실패 구간만 `WHISPER-WINDOW-QUARANTINED` 로 빠지며 나머지는 그대로 문서에 도달한다. `allow_incomplete` 면 반복 검사를 건너뛴다 — 라이프로그는 구간을 버리지 않는다 |
| `stt_window_store` | module | `skills/speechtotext/scripts/stt_window_store.py` | 구간 결과가 사는 곳 — `~/.hermes/speechtotext/windows/<키>/`(0700/0600) 가 성공한 구간을 남겨 재실행이 **이어서** 하게 하고, `quarantine/<키>/` 가 깨진 원본 payload 를 보존하며, `partial/<키>.md` 가 거부 직전의 부분 전사본 자리다. 캐시 루트가 git 체크아웃 안이면 fail-closed 거부(런타임 상태는 체크아웃 밖 규약) |
