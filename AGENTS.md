...
## OVERVIEW
cha의 개인 Hermes 에이전트 시스템. Discord로 타 연구자 에이전트와 상호운용하며, 설치별 `<primary-node>` / `<rag-node>` 역할로 구동한다. Python 3.12 **stdlib 중심**. 모든 대화·배치 모델 호출은 `openai-codex/gpt-5.6-sol` 구독 OAuth 단일 경로를 쓰며, 인증 또는 호출 실패는 fail-closed로 전파한다. **모든 외부효과는 소유자(cha) 승인 게이트 경유** — 이것이 아키텍처의 중심 불변식이다.
...
├── configs/      # routing-policy.md, peers.example.yaml, sensitivity-rules.yaml, external-effect-tools.yaml, rag/
...
| 라우팅·모델·예산 정책 | `configs/routing-policy.md`, `configs/*.yaml` | `openai-codex/gpt-5.6-sol` 단일 OAuth 티어, 구독은 별도 예산이며 두 번째 티어 없음, patent-sensitive=Codex OAuth 검증 필수 |
...
| `precheck` | module | `automation/skill_generation/precheck.py` | **스킬 제작 전 기존 스킬 대조 게이트**(t_0a7959e9, 2026-09-05) — 런타임/리포 `skills/`·governed live 마운트(`skill_mount.live_root`)·계정 자가 스킬 루트를 열거해 `SkillCard`(이름·설명·태그·트리거·`scripts/*.py`·회귀 유무·게이트 낱말)를 만들고, 후보 요청과 대조해 `REUSE-EXISTING | NEW` 를 결정적으로 판정한다(LLM 없음). 점수 정의는 복사하지 않고 `selfskill_audit.overlap`(`containment` ≥ 0.5, 겹친 낱말 ≥ 5)을 import 한다 — 사후 감사와 제작 전 게이트가 같은 자를 쓴다. `service.observe` 는 초안을 **쓰기 전에** 이것을 돌려 `REUSE-EXISTING` 이면 초안·W1-8 요청을 만들지 않고, `NEW` 면 생성 `SKILL.md` 에 `## 기존 스킬 대조`(열거 개수·가장 가까운 3개·차이·재사용 구성요소)를 싣는다. 증적은 판정 1건당 `reviews.jsonl` 한 줄(`enumerated` 전체 이름·`viewed` 실제로 읽은 SKILL.md 경로·`matches`·`skipped`)이고 `cli.py review NAME` 으로 되꺼낸다. 읽을 수 없는 SKILL.md 는 사유와 함께 `skipped` 로 남을 뿐 예외를 던지지 않는다. 벤더 `skill_manage(create)` 에는 훅 자리가 없어 그 경로는 사후 advisory 가 계속 탐지기다(`docs/guide/스킬-제작.md` §0, [소개](docs/기능소개/스킬-제작-전-기존-스킬-대조.md)) |
...
| `stt_eval` | pkg | `automation/stt_eval/` | 체크아웃 밖 오디오 SHA 원장·가설·소유자 교정 정답으로 CER·화자 순열 CER·DER·개체 지표를 비교한다. GPU 유휴·공유 lock 아래 후보 순차 실행, 마스킹 보고·채택 권고만 수행(자동 기본값 변경 없음). `cron/stt_eval_capture_watch.py`는 Drive·Obsidian 미러의 편집을 읽어 수집하며 원본·동결본·Discord 수신을 건드리지 않는다. [소개](docs/기능소개/전사-정확도-사용중-개선-루프.md) |
| `stt_attribute` | module | `skills/speechtotext/scripts/stt_attribute.py` | 단어 시각의 정수 밀리초 지지로 화자를 먼저 배정한 뒤 문장을 조립한다. BPE 낱말·겹침 참여자 보존, 직전 화자 상속 금지, 미상·겹침은 `화자0`(이름 배정 대상 아님). 세그먼트 전용 입력만 legacy 분할·배정 경로를 쓴다 |
| `audio_archive` | module | `automation/plaud_sync/audio_archive.py` | `DRIVE_PUBLISH_ENABLED=1`일 때 Plaud 원본 바이트를 Drive 공용 파사드로 보관하고 검증된 SHA만 비공개 `STT_EVAL_ROOT/manifest.jsonl`에 기록한다. 검증·기록 실패는 진단 후 로컬 원음 보존, 옵트아웃은 기존 삭제 정책. `audio_manifest.py`의 잠금·원장 계약을 회의 Drive 워처도 재사용한다 |
| `stt_engines` | service | `configs/stt-engines/` | pyannote 분리·WhisperX 문자 정렬 격리 CLI. 메인 stdlib 트리와 별도 venv·uv.lock, HF_TOKEN·모델 동의·체크아웃 밖 캐시 필수. 모델 준비 후 오프라인 재사용, 실패는 종료코드·센티널로 명시. 기본 전사 경로로 자동 승격하지 않는다 |
| `stt_split` | module | `skills/speechtotext/scripts/stt_split.py` | 순수 분할 단계 — 단어 시각 경로는 **배정 후** 긴 문장을 나누고, 세그먼트 전용 legacy 경로는 **배정 전**에 나눈다. 기존 분할 규칙은 `split_on_turns` 가 턴 경계(겹침 ≥1초)와 15초 마크에서 가장 가까운 띄어쓰기를 찾아 문장을 자른다. 문장 하나에 화자 하나를 주므로 문장이 화자보다 길면 분리 결과가 문서에 도달할 수 없다(실측: 구두점 없는 4인 57초 샘플이 158자 한 문장으로 나와 통째로 화자1). `stt_polish`·`stt_blocks` 는 구두점을 보지만 여기서는 **시각**을 본다. 자를 근거가 없어도 절대 예외를 던지지 않는다(fail-soft). **2026-09-06**: 띄어쓰기가 하나도 없으면 **글자 경계**로 자른다 — 토큰을 그대로 이어 붙이게 된 뒤(`stt_blocks`) 중국어·일본어에는 자를 자리가 영영 없어지고, 그러면 화자가 넷이어도 문장 하나라 전부 첫 화자에게 간다. 조각은 낱말을 다시 이어 붙이지 않고 **원문에서 잘라 내므로** 도로 이어 붙이면 원문과 정확히 같다 |
...
| `unique_session_name` | fn | `skills/mail/vendor/mailon/browser.py` | mailon 실행 하나에 브라우저 하나를 **구조로** 보장한다(2026-09-07) — agent-browser 는 `--session` 이름으로 브라우저 인스턴스를 잡으므로 고정 이름 `mailon-sync-send` 를 공유한 두 발송이 같은 탭에서 서로의 로그인 폼을 덮어쓰고 둘 다 `auth_error` 로 끝났다(13:06~13:09 UTC 실측, 로그인 8회 시도/6회 성공). 이름에 pid+난수를 넣어 sync·send·resolve 세 팩토리 전부가 자기 세션을 열고 `ephemeral=True` 로 표시해 `close` 가 자기 `<name>.config` 만 치운다. 비용은 없다 — 노드 실측으로 세션 신설은 0.9초이고 `close` 가 크롬 프로필을 지우므로 **쿠키는 실행을 넘어 남지 않는다**(같은 이름으로 재개해도 `document.cookie` 가 비어 있다). 즉 "세션 만료" 는 이 시스템에 존재한 적이 없는 실패 원인이다 |
| `send_lock` | module | `skills/mail/vendor/mailon/send_lock.py` | **실제 발송만** 직렬화하는 크로스 프로세스 lock(`<runtime>/data/send.lock`, `fcntl.flock`) — 세션 격리가 동시 발송의 상호 파괴를 없애면서 드러난 이중발송 위험을 닫는다. 발송 전 중복검사(`send.DUPLICATE_SUPPRESS_WINDOW_MS`)는 check-then-act 라 실측에서 두 프로세스가 **65ms 차로 나란히 통과**했다(13:07:13.718/.783). lock 을 잡은 뒤에야 그 검사가 의미를 갖는다 — 진 쪽은 기다렸다가 이긴 쪽의 메일을 보낸편지함에서 발견해 duplicate-suppressed 로 끝난다. dry-run 은 외부효과가 없어 직렬화하지 않고, 대기 상한(기본 300초) 초과는 `SendLockBusy`(=`SendSafetyError`, 새 error_code 없음)로 fail-closed. class 기반 CM 인 이유는 `pipeline_lock.hold` 와 같다 |
| `triage_llm_routing` | module | `skills/mail/scripts/triage_llm_routing.py` | 다이제스트 Codex OAuth 단일 경로 — `LlmUnavailableError`는 호출 실패로 전파하며, `llm-calls.jsonl` 표식과 소유자 DM의 단일 경고 줄을 남긴다 |
| `recall` | module | `skills/recall/scripts/recall_cli.py` | RAG 검색 (v2: patent-sensitive는 Codex OAuth 경로 기계검증 시만 센티널 부착 포함, 그 외 제외 — 경로 미검증은 provider call 전에 거부) |
...
| `wizard` | module | `automation/install/wizard.py` | 첫 설치 질문·node.toml 작성·계획 출력 집계·결과 안내. 설치 로직 없이 기존 설치기 argv만 조립하며, sudo는 `yes` 확인(비대화 `--yes`) 뒤에만 실행한다. [소개](docs/기능소개/설치-마법사.md) |
| `profiles` | module | `automation/install/profiles.py` | 프로필→헬스체크 선언·컴포넌트의 단일 정의. `core`·`rag`는 컴포넌트 없이 주 노드 선언만 하고, `report-hub`·`full`은 `report-hub`를 함께 고른다(`rag` 배포는 별도 `RAG_NODE`). 설치기가 `/etc/autophagy/healthcheck.env`(root:root 0644)를 쓰고 registry는 환경변수가 unset일 때만 파일로 fallback한다. 생략은 기존 계획 유지, `--suggest` 는 복구용(기존 노드·수동 서비스 변경 뒤)이다 |
| `healthcheck_probe_asset` | module | `automation/install/healthcheck_probe_asset.py` | healthcheck SSH 강제명령 자산 설치 액션. ops 전용 키·운영자 `authorized_keys` 바인딩·래퍼 지문을 검사하고, 불완전 키나 생성 확인 실패는 fail-closed로 멈춘다 |
| `report_hub_component` | component | `automation/install/{components.py,component_assets.py}` | `report-hub` opt-in 사용자 유닛 컴포넌트. ops 작업 디렉터리·릴리스 `automation` 링크·비밀 없는 `hub.env`·두 유닛을 수렴하며, 명시하거나 해당 프로필을 고를 때만 배치한다 |
| `OPT_IN_COMPONENTS` | registry | `automation/install/components.py` | 설치기 opt-in 컴포넌트 단일 레지스트리. **이름을 대지 않으면 파일도 타이머도 생기지 않는다**(disabled가 아니라 absent). `report-hub`는 ops 사용자 유닛·환경 선언을 포함하며, 멱등성은 기존 `build_plan`의 digest·활성타이머 대조에서 그대로 온다 |
...
| `proposal_engine` | pkg | `skills/proposal/engine/` | HWPX 렌더 엔진의 **단일 정의**(13,670 LOC). 2026-09-08 소유자 지시로 외부 private repo `kimm-docbot` 에서 내제화했다 — `.omo/plans/proposal-improve.md` M9("KD를 pinned-SHA subprocess 로 쓴다")를 **의도적으로 뒤집은 것**이고, 이유는 엔진 한 줄 수정의 고정비가 4단 의식(엔진 커밋·푸시 → `PROPOSAL_DOCBOT_PIN` 전진 → PR → 노드 env 갱신)이었기 때문이다. **stdlib 전용**이라 스코프 venv 도 AGENTS.md 서드파티 예외도 없다 — pydantic 은 `contracts/frozen_model.py` 의 frozen dataclass + JSON 코덱으로, `anthropic` 은 `agents/hermes_client.py`(Codex OAuth 단일 경로)로, `docx`/`pypdf` 는 `automation.document_text` 위임으로 대체했다. 핀·`ENGINE-PIN-BLOCK` 은 없어졌고 산출물의 `engine_sha` 는 엔진 바이트의 digest(`engine.render.engine_digest`)다. 경계는 산문이 아니라 `tests/unit/test_proposal_engine_pin.py` 가 기계로 강제한다 |
| `term_correction` | module | `automation/term_correction.py` | 산출 문서의 용어를 바로잡는 **단 하나의 엔진** — 참고 문서 파싱(한 칸=바른 용어, 두 칸=소유자가 지목한 치환, 옛 `틀린표기=올바른표기`), 지목 치환, 그리고 바른 용어와의 **자모 OSA 거리**로 어절 **앞머리만** 고치는 근접 교정. 세 음절 미만·음절 수 불일치·두 용어에 동시 근접이면 손대지 않는다(「성금=선금」이 「기성금」을 깨뜨린 사고의 가드). 교정이 전사본이 아니라 산출 문서에서 일어나므로(「용어 교정 위치 규칙」) 스킬 밖 `automation` 에 있고, 사본은 `tests/unit/test_term_correction_conformance.py` 가 막는다 |
| `term_glossary` | module | `automation/term_glossary.py` | 교정 참고 문서의 **문서 종류별 중첩 층** — `autophagy/용어집.csv` → `autophagy/<문서 종류>/용어집.csv` → `…/<과제>/용어집.csv` 를 바깥부터 병합해 깊은 층이 같은 낱말을 덮는다. 폴더 이름의 정본은 `drive_taxonomy.CATEGORIES` 이고 Drive 산출물이 아닌 종류만 `EXTRA_FOLDERS`(lifelog→라이프로그)가 채운다. 조회는 `find_folder_path` 라 **찾는 일이 폴더를 만들지 않고**, `DRIVE_PUBLISH_ENABLED` 옵트인이며, 한 층이라도 실패하면 부분 병합 대신 노드 캐시로 내려간다(부분 병합은 안쪽 덮어쓰기를 조용히 흘린다). Drive 가 "없다"고 답하면 캐시를 비운다 |
| `term_correction_log` | module | `automation/term_correction_log.py` | 어느 문서에서 무엇이 무엇으로 바뀌었는지 **어절만** 남기는 JSONL(`at·document·label·project·stage·kind·term·before·after`, 0700/0600, 체크아웃 밖). 문맥을 함께 남기면 문서 본문이 로그로 샌다. 쓰기 실패는 `CORRECTION-LOG-FAIL` 한 줄일 뿐 문서 생성을 멈추지 않는다 |
...
| `plaud_sync` | pkg | `automation/plaud_sync/` | Plaud lifelog 동기화 — MCP 를 에이전트에 등록하지 않고(no-agent cron 이 `npx @plaud-ai/mcp@0.3.10` 과 stdio JSON-RPC 로 직접 대화, 버전 고정) 녹음 건별 노트(`## 요약` 위+`## 전문` 아래)를 `~/.hermes/plaud-sync/notes/` 에 동결, 건별 소유자 ✅(OBSIDIAN_WRITE 재사용·요청별 스레드) 후 `obsidian_write.write_note` 로 push 하고 RAG 가 자동 소비한다. 본문 sha 불일치는 게시·push 모두 거부(fail-closed), 회의록 체인은 자동 호출하지 않는다. cron `plaud_sync_watch`(10분 틱, Plaud 폴은 내부 30분 게이트). [소개](docs/기능소개/plaud-lifelog-동기화.md) **2026-09-02 v2**: 카드가 동결 본문 `## 요약`(없으면 `## 전문`) 상위 5줄을 인용한다(`render.summary_preview`, 바인딩 해시 불변). 카드 **형식**만 바뀌면 파사드는 재게시하지 않으므로 `plaud_sync_watch.py --repost-posted` 가 정상 틱(✅ 소비)→옛 카드 삭제·planned 복귀(`repost.py`)→틱 순으로 **같은 스레드**에 다시 올린다 — `agent_chat_request_thread` 는 조회 없이 매번 새 스레드를 만들기 때문에 `effects_live.thread_candidates` 가 레코드의 `approval_thread_id` 를 재사용 후보로 넣는다. 상태 조회는 governed 스킬 `skills/plaud`(`plaud_cli.py status`, stdlib·읽기 전용, 트리거 "plaud 상태"). **2026-09-04 v3(로컬 전사)**: 발견은 `transcribing` 으로 동결하고, 틱이 watch.lock 을 **푼 뒤** `transcribe_live` 가 `pipeline_lock`(speechtotext 와 공유) 아래 `get_file` presigned URL 로 오디오를 내려받아 speechtotext CLI(`SPEECHTOTEXT_BACKEND=local` 고정·`DRIVE_PUBLISH_ENABLED=0`)로 전사한 뒤 `transcripts/<stem>.md` 에 남기고 노트 `## 전문` 을 그것으로 재조립해 `planned` 로 올린다(commit 은 watch.lock blocking 재획득 + 상태·hash 재검사). 환경 실패(rc 3/4·MCP·네트워크)는 무카운트 재시도, 녹음 실패는 2회면 클라우드 전사 폴백(출처 줄에 명시). 킬스위치 `PLAUD_SYNC_TRANSCRIBE=0`. [소개](docs/기능소개/plaud-녹음-로컬-전사.md) **2026-09-04 노트 v2 양식(B안)**: 노트가 소유자 Obsidian Linter 가 그대로 두는 frontmatter(tags→title→source→created→modified, 필요할 때만 따옴표 — vault 의 실제 플러그인 빌드로 61개 제목·렌더 샘플 3종을 헤드리스 멱등 검증, `docs/qa/PLV2`)로 시작하고 `## 한눈에`(녹음·주제·사람·장소·한 줄 Dataview 필드) → `## 요약` → `## 결정 · 할 일` → 접힌 `## 전문` → 출처. 사람·장소·결정·할 일은 `lifelog_extract_live.build_extractor`(규칙 파일→patent-sensitive→Codex OAuth 클라이언트→템플릿 순 게이트, provider `openai-codex`)가 **로컬 전사가 들어온 `transcribe.finalize`** 에서 뽑고(클라우드 초안은 LLM 미호출), 생략 사유는 한눈에 줄에, 전송·파싱 실패는 무카운트 대기. `obsidian_write.render_note` 는 body 선두 frontmatter 를 제목 위로 올리고 callout 을 생략한다(그 외 노트 바이트 동일). 카드 v3 는 한눈에 줄을 먼저 인용. [소개](docs/기능소개/plaud-lifelog-노트-v2-양식.md) **2026-09-04 v4(구간 전사·빈 노트 차단, t_4e3d6630)**: 로컬 전사가 구간별로 돌아 한 구간의 결함이 전사 전체를 버리지 못하고(`stt_window*`), 자식 env 에 `SPEECHTOTEXT_ALLOW_INCOMPLETE=1` 을 더해 표식 있는 부분 전사본을 받으며, `transcribe._fallback` 은 요약·전문이 **둘 다** 비면 승격 대신 blocked 로 남긴다 — 요약도 전문도 없는 노트가 vault 에 들어가는 경로는 닫혔다. [소개](docs/기능소개/plaud-긴-녹음-구간-전사.md) **2026-09-05 v5(실동작 검증)**: 발견이 클라우드 요약·전사 둘 다 빈 녹음을 `initial_status` 와 무관하게 skip 해(75429f497 의 빈 노트 가드) 로컬 전사가 가장 필요한 녹음이 영원히 발견되지 않았다 — 64분 녹음 실측. transcribing 경로에서는 동결하고, 전사 후보는 **시도 횟수 적은 순**(상한에서 `클라우드 폴백 보류` 된 옛 녹음이 틱당 1건을 독점해 새 녹음을 굶기지 않게), `audio.download` 는 받은 바이트가 Content-Length 에 못 미치면 거부한다(`http.client` 는 조기 종료에 `IncompleteRead` 를 내지 않는다). 노드 첫 로컬 전사 실측: 64분 → CUDA 약 6분, 블록 529, 격리 0, 카드 posted. **2026-09-05 v6 정책 게이트(PR #414)**: 보류 재전사는 선택 필드 `next_transcribe_at`으로 1h→2h→4h→8h→16h→24h(상한) 백오프(`transcribe_policy`), 클라우드는 로컬 슬롯 밖에서 매 틱 재확인하며 `PLAUD_SYNC_TRANSCRIBE_GIVE_UP`(기본 5회)에도 비면 `abandoned`+저널로 끝낸다. `PLAUD_SYNC_MIN_DURATION_MS`(기본 5000) 미만은 발견 시 저널만 남기고 skip(레코드·카드 0), 이미 transcribing이면 다음 처리에서 abandoned(`duration`). `prompts/lifelog-extraction-v2.md`가 기존 추출과 **같은 호출**에서 summary를 받고 Plaud 요약이 있으면 그것이 우선한다(추가 호출 0·민감도 게이트 불변). [소개](docs/기능소개/plaud-전사-정책-게이트.md) **2026-09-06 v7(스윕 6)**: 200–250 LOC 구간 파일을 책임별로 갈랐다 — `transcribe.py`→`transcribe_promote.py`, `transcribe_live.py`→`transcribe_live_effects.py`(PR #418), `note.py`→`note_paths.py`, cron 래퍼→`watch_runtime.py`(PR #419); 공개 import 경로·deploy-manifest·emit 된 워처 매니페스트는 그대로다. PR #424: `get_file` 이 오프셋 없는 UTC 문자열로 주는 `created_at`·`start_at` 을 `audio.parse_source`(및 `transcribe_text.recording_from_source`) 경계에서 `+00:00` 으로 정규화해 로컬 전사 노트의 frontmatter·`녹음::`·출처 줄이 제목·경로와 같은 현지 시각을 쓴다(클라우드 노트 출력·저장된 state 불변). PR #424 이전에 쓰인 vault 노트 3건의 보정은 OWNER 이며 `--repost-posted` 는 본문을 재생성하지 않는다 **2026-09-06 요약 누락 차단**: 첫 추출이 요약을 배열로 답해도 받고(`_summary`), Plaud 요약도 첫 추출도 요약을 못 주면 `prompts/lifelog-summary-v1.md` 로 요약만 한 번 더 묻는다(`summarize` — 요약이 있는 정상 경로에서는 호출 수 불변). 복구가 실패해도 이미 얻은 사람·장소·결정은 버리지 않고, 그래도 없으면 한눈에 줄이 `- 요약:: 없음 (모델이 전사본에서 요약을 만들지 못함)` 으로 사유를 적어 게이트 생략(`- 추출:: 생략`)과 구별된다. 복구 응답이 JSON 도 불릿도 아닌 산문이면 요약으로 받지 않는다 — 거절문이 실린 노트는 빈 요약보다 나쁘다. [소개](docs/기능소개/plaud-lifelog-요약-누락-차단.md) **2026-09-06 재처리**: `written`·`abandoned` 은 틱도 `store.pending()` 도 다시 보지 않는 종결 상태라 낡은 코드가 만든 노트를 다시 만들 길이 없었다. `repost.reset_for_reprocess`/`reprocess` 와 워처 플래그 `--reprocess <녹음 id>` 가 그 레코드 하나만 `transcribing` 으로 되돌려 다운로드→전사→추출→카드를 다시 돌린다. 노트 경로가 녹음 id 에서 나오므로 ✅ 하면 같은 파일이 덮어써지고, 카드는 기존 `approval_thread_id` 스레드로 간다. 옛 카드는 지우지 않는다(소유자가 누른 결정의 기록). 진행 중 상태에는 듣지 않고 자동 스윕도 아니다 — GPU 수 분과 새 ✅ 를 쓰므로 소유자가 부르는 명령이다. |
| `transcribe_promote` | module | `automation/plaud_sync/transcribe_promote.py` | `transcribe` 파사드에서 떼어낸 **승격·폴백 조립**(2026-09-06, PR #418) — 로컬 전사본 변환과 클라우드 폴백 승격을 한 이음새로 소유한다. 파사드는 원래 import 표면을 유지한다(115 pure LOC) |
| `transcribe_live_effects` | module | `automation/plaud_sync/transcribe_live_effects.py` | `transcribe_live` 의 **구체 효과**(2026-09-06, PR #418) — `LiveEffects` 의 MCP·파일시스템·subprocess·lock 구현이 여기 있고 실행 파사드는 호환 이음새만 남긴다(199 pure LOC) |
| `note_paths` | module | `automation/plaud_sync/note_paths.py` | 라이프로그 노트의 **안정된 이름·목적지**(2026-09-06, PR #419) — `note.py` 에서 떼어낸 슬러그·경로 산술(64 pure LOC) |
| `watch_runtime` | module | `automation/plaud_sync/watch_runtime.py` | cron 래퍼 `plaud_sync_watch.py` 의 **런타임 설정·안전 헬퍼**(2026-09-06, PR #419) — lock·env 해석을 소유하고 래퍼 부트스트랩은 그대로다(80 pure LOC) |
| `mcp_transport` | module | `automation/plaud_sync/mcp_transport.py` | Plaud MCP의 **stdio 전송 절반**(2026-09-05, PR #410) — `StdioMcpTransport`가 자식 프로세스·reader 스레드·JSON 프레이밍·EOF/오류 이벤트를 소유하고, `mcp_client.PlaudMcpClient`는 초기화·요청 응답 프로토콜 파사드로 남는다. 두 파일은 각각 155 pure LOC이며 `PlaudMcpClient`·`PlaudMcpError`·JSON 타입의 공개 import 경로는 그대로라 호출부가 분리를 알 필요가 없다 |
...
| `provision-skill-roots.sh` | script | `automation/provision-skill-roots.sh` | 스킬 루트 토폴로지 **반전** 프로비저너(멱등, root 실행) — 레거시 read-only bind 해제와 fstab 2행 제거, agent 1차 루트를 0700 agent 소유로, `.hub` 상태를 taps.json sha256 readback과 함께 이관, config에 `external_dirs`+`guard_agent_created` 패치(기존 `skills:` 블록은 넘겨짚지 않고 `SKILLS-BLOCK-BLOCK`으로 멈춤), 끝에 `hermes skills list`로 external 발견 선검증. peer arm은 잔여 사본을 3중 조건(이름·author 마커·repo 존재)으로만 정리하고 현역 자가 스킬 1개(`autophagy-interop`)를 pin — `skill-deploy-review` 는 **pin 하지 않는다**: E7(`docs/patch/2026-07-17-e7-peer-attestation.md`)이 "an agent must not read a Discord request and be instructed to run a reviewer as part of deployment" 로 은퇴시킨 리뷰어인데 2026-08-15 반전이 보호 목적으로 pin 목록에 넣어 매 프로비저닝이 되살렸고, 그러나 그 회수(2026-09-04)만으로는 `[release]` 카드의 상시 거짓 ⛔ 가 멎지 않았다 — 심사를 지시한 문서는 **pin 된 `autophagy-interop`** 이 2026-09-01 에 자가 저작한 `[release]` 절차였고(노드 미러의 origin/main 은 릴리스 수렴 뒤에야 전진하므로 새 릴리스 HEAD 는 언제나 "unpushed tip"), 2026-09-05 에 peer 게이트웨이의 `discord.ignored_channels`(A) + 그 절차 제거(C)로 닫았다([patch](docs/patch/2026-09-05-peer-gateway-ignores-approvals.md)). 회귀는 `test_provision_when_the_retired_reviewer_is_present_then_it_is_not_pinned` 가 **존재**를 전제로 고정한다(부재면 `PEER-PIN-SKIP` 이라 고치지 않아도 통과) — [patch](docs/patch/2026-09-04-peer-retired-reviewer-unpin.md) |
...
| `stt_blocks` | module | `skills/speechtotext/scripts/stt_blocks.py` | 전사본 본문의 **문법 한 곳** — 블록(빈 줄로 구분) + 한 줄에 한 문장, 헤더 `[HH:MM:SS] 화자N · 이름`. whisper 토큰 오프셋을 문장 구간으로 되살려 각 블록이 언제 시작했는지 적는다(예전 경로는 구간을 한 문자열로 이어 붙이며 그 값을 버렸다). `render()` 가 쓰고 `parse()` 가 되읽어 디스크의 옛 문단 전사본과 새로 다듬은 전사본이 같은 문서로 수렴한다. 문단 규칙(4문장·180자)은 **화자 배정 여부와 무관하게** 블록을 끊는다 — 화자가 하나뿐인 라이프로그가 문장 179개를 블록 하나로 내던 것을 2026-09-07 에 고쳤다(옛 규칙은 화자가 없을 때만 적용됐다). **2026-09-05 PR #411**: 반복 의심 창의 HTML `<details>`를 `folded` 불투명 증거로 붙여 render/parse 왕복·재다듬기·화자 이름 반영에도 내부 원문을 그대로 보존한다 — 실패 표식과 접힌 증거는 신뢰할 발화로 다시 분할하지 않는다 **2026-09-06**: 토큰을 **그대로 이어 붙인다** — whisper 는 낱말 첫 조각에만 앞 공백을 붙이므로 조각마다 공백을 넣던 옛 규칙이 `통신 규약` 을 `통 신 규 약` 으로 부쉈다(노드 실측 133초 한국어: 이어 붙이기가 `segment.text` 와 36/36 일치, 옛 규칙 0/36, 가짜 공백 198개). 세그먼트 첫 낱말에만 whisper 가 빠뜨렸을 때 공백을 넣어 앞 문장의 마침표가 다음 발화와 붙지 않게 한다 |
| `stt_diarize` | module | `skills/speechtotext/scripts/stt_diarize.py` | 화자 분리 — sherpa-onnx 바이너리를 노드에서 실행(**음성은 노드 밖으로 나가지 않는다**), 진단 섞인 stdout 에서 turn 줄만 파싱, 단어 시각이 있으면 `stt_attribute`가 배정 후 문장을 조립한다. 세그먼트 전용 legacy 경로만 문장 겹침 최대 turn → 2초 내 최근접 turn을 쓰며, 무근거는 직전 화자 상속 대신 `화자0`이다. `SPEECHTOTEXT_DIARIZE_BACKEND=pyannote`는 격리 CLI를 쓰는 옵트인이고 sherpa가 기본이다. 도구·모델이 없거나 실패하면 `DIARIZE-FAIL <사유>`(stderr) 후 화자 없이 계속하는 fail-soft이며, 전사를 절대 중단시키지 않는다. **2026-09-05 PR #405**: `SPEECHTOTEXT_DIARIZE_MAX_SPEAKERS`(기본 8)를 넘기면 `DIARIZE-OVERSEGMENTED`로 화자 배정만 fail-soft한다. **2026-09-06 PR #422**(2026-09-07 에 초과됨): 그때 `DEFAULT_THRESHOLD=1.35` 였다 — 소유자가 두 화자로 확인한 64분 표본에서 1.30의 세 번째 군집이 19.5%를 차지해 무시할 수 없었고, 1.35는 71.5%/28.5%의 두 군집이었다. 다만 그 표는 `--min-duration` 가드를 **주지 않고** 잰 것이라 실행 조건과 달랐다. cap 판정 전에 `DIARIZE-CLUSTERS speakers=<N> threshold=<T> turns=<N>`을 남긴다. 1.40은 한 군집으로 합쳐지는 과병합 위험값이며, 화자 수를 알면 `--speaker-count`를 쓴다. **2026-09-06 (이 사이클)**: 상한을 넘겨도 **라벨을 버리지 않는다** — `--clustering.num-clusters=<상한>` 으로 한 번 다시 묶고(`DIARIZE-RECLUSTERED`), 그래도 넘칠 때만 `DIARIZE-OVERSEGMENTED` 로 화자 없이 간다. 그 플래그는 "정확히 k" 가 아니라 **"최대 k"** 라 화자를 지어내지 않는다(133초 샘플 k=3·4·6·8 → 모두 2, 데이터가 받쳐주는 15분 회의 k=3·4 → 3·4). 파편 가드 `--min-duration-on/off`(**0.5/0.0**, `SPEECHTOTEXT_DIARIZE_MIN_SPEECH`·`_MIN_SILENCE`)를 넘긴다 — 이름만 닮았고 `on` 은 그 길이 미만 조각을 **버리고** `off` 는 같은 화자의 그 간격 미만을 **재귀적으로 이어 붙인다**. 둘 다 군집 수는 바꾸지 않는다. `SPEECHTOTEXT_DIARIZE_SPEAKERS` 로 아는 화자 수를 못박으면 임계값 추정도 상한 보수도 돌지 않는다. **2026-09-07**: `DEFAULT_THRESHOLD=1.0` · `off=0.0` · 잔여 군집 프룬(`substantial`, `RESIDUAL_SHARE_FLOOR` 5%). 소유자가 화자 3·4명이라 말한 라이프로그 두 녹음이 1.35 에서 **둘 다 화자 1명**으로 나온 것이 계기다(4.5분 녹음 턴 12개·중앙값 16.65초 — 화자 수 이전에 발화 구분이 없었다). 임계값 사다리는 절벽이고(272.5초 0.8:6 · **1.0:4** · 1.2:2 · 1.35:1) **세 녹음을 동시에 맞히는 고정값은 없다** — 파이프라인 출력 기준 오차 합이 1.35 는 5, 1.0 은 3 이다(272.5초 1→4 · 549.4초 1→4 · 64분 2→4). 1.0 을 고른 이유는 총오차 말고도 **실패의 방향**이다: 1.35 의 실패는 과병합이라 화자가 문서에서 사라져 복구할 수 없고, 1.0 의 실패는 과분할이라 `--speaker-count` 로 되돌릴 수 있다. `off` 는 세 녹음 전부에서 화자 수를 바꾸지 않고 턴만 늘렸다(40→82 · 139→193 · 584→998). 증적 `docs/qa/PLD1/summary.md`, `docs/qa/FU6/diarize-threshold-validation.md`, `docs/qa/PLQ1/summary.md`, [소개](docs/기능소개/라이프로그-화자-분리-정정.md) |
...
| `stt_speaker_count` | module | `skills/speechtotext/scripts/stt_speaker_count.py` | 전사본이 말해 주는 화자 수를 **얼마나 믿을지**만 정하는 순수 판정 — 모델도 파일도 부르지 않는다. `parse_count` 는 정수 하나만 답으로 받고 "3~4"·"3명"·산문·범위 밖은 전부 거부한다(애매한 답에서 숫자를 주워 오면 모델이 망설인 자리가 문서에서 누군가의 발언이 된다). `should_redo` 는 답이 지금 화자 수와 다를 때만 재분리를 시킨다 — 같은 답에 분리기를 또 돌리는 것은 시간만 쓴다. 근거: 화자 수를 **주면** pyannote 가 272.5초 k=3 → 45.6/42.9/**11.4**%, 549.4초 k=4 → 44.0/22.3/17.8/**15.8**% 로 기준점에 도달한다(자동은 둘 다 2명). 즉 모자란 것은 분리기가 아니라 k 를 아는 일이고 그 근거는 오디오가 아니라 전사본에 있다. `unlabelled` 은 그 질의 **직전에** 블록 헤더의 화자 라벨을 걷어낸다(시각과 말은 그대로) — 같은 오디오·전사·프롬프트로 잰 A/B 에서 라벨을 남긴 초안은 2명, 걷어낸 초안은 3명(기준점)이었다. 초안이 1차 분리 라벨로 조립되므로 모델은 보이는 라벨 종류를 그대로 세어 돌려주고, 프롬프트의 "라벨을 믿지 말라" 는 그 앵커를 이기지 못한다 |
| `stt_speaker_ask` | module | `skills/speechtotext/scripts/stt_speaker_ask.py` | 그 질의의 **실행 경계** — `SPEECHTOTEXT_SPEAKER_COUNT_LLM=1` 옵트인이고 미설정이면 `resolve()` 가 None 이라 전사가 예전과 바이트 그대로다. 초안이 `patent-sensitive` 로 분류되면 모델을 부르지 않고(`RECOUNT-SKIP`), 민감도 규칙을 읽지 못해도 묻지 않는다(`RECOUNT-FAIL rules-unreadable`) — 화자 수를 세자고 게이트 없는 모델 경로를 새로 열 수 없다. 호출은 저장소의 유일한 경로 `automation.codex_llm` 이고 다른 provider 로 내려가지 않는다. 프롬프트 자산 `prompts/speaker-count-v1.md`, 배선은 `stt_local.transcribe(count_speakers=…)` |
| `stt_speaker_flow` | module | `skills/speechtotext/scripts/stt_speaker_flow.py` | 전사 → 다듬기 → 이름 → meeting 자식 호출을 잇는 배선. meeting `ingest` 의 **마지막 stdout JSON** 의 `speakers` 만 받아들이고, 범례가 실제로 바뀔 때만 전사본을 다시 쓴다(정본은 하나). 두 패스 모두 낱말을 바꾸지 않으므로 다시 다듬어도 전사본의 말은 그대로다 |
| `stt_polish` | module | `skills/speechtotext/scripts/stt_polish.py` | 전사본을 **읽을 수 있게** 다듬는다(요약 아님) — 문장 분할 후 `stt_blocks` 블록으로 묶기(한 줄 한 문장 + 화자·시각 헤더 + 머리말 화자 범례), 연속 완전중복만 접기. **낱말은 바꾸지 않는다**(2026-09-05 「용어 교정 위치 규칙」) — 용어집은 인식 **전** `--prompt` 힌트로만 쓰이고, 교정은 회의록·라이프로그를 만들 때 `automation/term_correction` 이 한다. 해석(결정사항·액션아이템)은 meeting 이 소유하고 이 모듈은 그 경계를 넘지 않는다 |
...
| `stt_window` | module | `skills/speechtotext/scripts/stt_window.py` | 긴 녹음을 **구간으로 나누는 순수 산술** — `plan_windows`(stride=창−겹침, 마지막 구간이 끝까지, 창보다 짧으면 구간 1개=예전 단일 실행), `decode_payload`(strict utf-8 → 실패하면 `errors="replace"` 로 다시 읽고 대체 문자 수를 함께 돌려준다 — **한 바이트가 두 시간을 죽이지 못하게 하는 지점**), `merge`(구간 i 는 `[i*stride,(i+1)*stride)` 에 시작하는 세그먼트만 소유해 이음매 중복이 0), `gap_marker`(실패 구간 자리의 한국어 표식 — `stt_blocks` 가 문장으로 끊도록 마침표로 끝난다), `cache_key`. I/O·subprocess·시계 없음 |
| `stt_window_run` | module | `skills/speechtotext/scripts/stt_window_run.py` | **구간당 whisper.cpp 프로세스 하나** — 정규화된 wav 를 `-ot <ms> -d <ms>` 로 구간만 읽고(슬라이스 파일 0, whisper 가 절대 오프셋을 보고하므로 시간축 보정 불필요), 예산도 구간 단위다. `accept()` 가 payload 를 판정해 `invalid-json`·`no-transcription`·`repetition=…` 사유를 돌려주고 실패 구간만 `WHISPER-WINDOW-QUARANTINED` 로 빠지며 나머지는 그대로 문서에 도달한다. `allow_incomplete` 면 반복 검사를 건너뛴다 — 라이프로그는 구간을 버리지 않는다. **2026-09-05 PR #411(speechtotext 1.2.2)**: 반복 의심 창은 `folded_reason`으로 보존해 기존 실패 표식 아래 시각·사유가 있는 HTML `<details>`로 접는다(FOLDED). 파싱 불가 등은 표식만 남고 원본 payload 격리·저널은 유지한다 — 반복 의심 텍스트를 버리지 않되 정상 발화로 승격하지 않는다 **2026-09-06**: 설치가 켠 whisper 디코딩 노브를 argv 끝에 실어 나른다(`decode_flags`) — `SPEECHTOTEXT_WHISPER_BEAM_SIZE`(-bs)·`SPEECHTOTEXT_WHISPER_NO_SPEECH`(-nth)·`SPEECHTOTEXT_WHISPER_SUPPRESS_NONSPEECH`(-sns)·`SPEECHTOTEXT_VAD_MODEL`(--vad -vm). 아무것도 켜지 않으면 argv 는 예전과 **바이트 동일**하고, 오타·범위 밖 값·없는 VAD 모델 파일은 조용히 무시돼 whisper 자신의 기본값이 선다. 무음 환각(짧은 녹음이 '감사합니다' 한 줄로 붕괴, 2026-09-05 실측)에 대한 whisper 문서의 완화책을 코드 변경 없이 시험하기 위한 자리이며, VAD 는 조용한 한국어 발화를 잘라내므로 여전히 기본이 아니다 |
| `meeting_extract.strip_folded` | fn | `skills/meeting/scripts/meeting_extract.py` | 회의 추출의 **프롬프트 입력 경계**(2026-09-05, PR #411 · meeting 1.7.1) — `meeting_llm.build_prompt`가 `{{MEETING_TEXT}}`를 채울 때만 HTML `<details>`(중첩·미닫힘 포함)를 제거해 반복 의심 전사를 LLM 근거로 쓰지 못하게 한다. 민감도 검사 입력과 부록 전사 원문은 그대로이며, 원문 파일을 다시 쓰거나 전사 블록 문법을 복제하지 않는다 |
| `stt_window_store` | module | `skills/speechtotext/scripts/stt_window_store.py` | 구간 결과가 사는 곳 — `~/.hermes/speechtotext/windows/<키>/`(0700/0600) 가 성공한 구간을 남겨 재실행이 **이어서** 하게 하고, `quarantine/<키>/` 가 깨진 원본 payload 를 보존하며, `partial/<키>.md` 가 거부 직전의 부분 전사본 자리다. 캐시 키는 오디오·모델·도구 지문·창 계획에 ASR 지문(언어·실제 prompt·문맥·디코딩 플래그·DTW·부분 전사 허용)을 더한다. `--dtw`는 `SPEECHTOTEXT_WHISPER_DTW` 명시 때만 켠다. 캐시 루트가 git 체크아웃 안이면 fail-closed 거부(런타임 상태는 체크아웃 밖 규약) |
...
| `meeting_terms` | module | `skills/meeting/scripts/meeting_terms.py` | 회의록 **본문만** 고치는 순수 계층 — 렌더 **전** `Extraction` 의 사람이 읽을 문장(제목·참석·요약·결정·액션·마일스톤·논의·미결·다음 회의)에 `automation.term_correction.apply` 를 걸고, 전사본 인용(`basis`)·관리번호(`ResolvedAction.id`)·화자 라벨·날짜는 지나간다. 렌더된 문서에 걸면 부록의 원문 전사본까지 고쳐져 증거가 사라지므로 자리가 여기다. 층 조회와 로그는 부작용을 가진 `meeting_cli._correct_terms` 의 몫 |
...
| `meeting_slides` | module | `skills/meeting/scripts/meeting_slides.py` | 발표자료(pdf/pptx/md/txt) 텍스트 추출 — 대명사·모호 지시어 교정 재료. **fail-soft**(스캔본·미지원·부재는 예외가 아니라 `Deck.status`로 돌아오고 ingest는 계속) + **fail-closed**(`gate_text()`로 회의 본문과 함께 민감도 게이트에 합산 — 빠뜨리면 특허 슬라이드는 Codex OAuth 검증 전에는 호출되지 않는다) |
| `selfskill_audit` | pkg | `automation/selfskill_audit/` | 자가 스킬 사후 감사 — `ledger.audit()`가 계정 스킬 루트와 `.archive`를 스캔해 콘텐츠 해시(`skill_review.skill_digest`) 기준 델타(created/edited/archived/restored/removed — 아카이브 없이 사라진 것도 보고)를 append-only `ledger.jsonl`에 적재, `report.run_once()`가 미보고분만 마스킹 요약으로 소유자 DM한 뒤 watermark를 전진시킨다(전송 실패시 전진 없음). 아침 보고에는 기능 겹침 advisory(SC-4, `overlap.py` — 다른 이름·같은 기능 자가 스킬을 description·tags 토큰 containment ≥0.5·겹침 ≥5 로 `OVERLAPS-GOVERNED:<skill>` 보고, 오탐 상한은 governed 18개 상호 대조 0건으로 회귀 고정)가 함께 실린다. cron `selfskill_audit_watch.py`(agent·peer 각 09:00 no-agent). **2026-09-05 PR #408**: `claims.py`가 SKILL.md 본문에서 승인 kind·판정 어휘·`ApprovalSurface`로 지연 유도한 채널 토큰을 찾아 `CLAIMS-APPROVAL-ROLE:<skill>` advisory를 아침 보고에 싣는다. governed corpus 0-hit 회귀(`tests/unit/test_selfskill_claims.py`)가 오탐 상한을 지키며, 자가 저작 차단이나 승인 심사 권한은 만들지 않는다 |
...
| `deploy_all` | script | `automation/deploy_all.sh` | origin/main **전량 수렴** 오케스트레이터(RC-3/4) — 판정은 노드 릴리스 트리의 `deploy_all_probe.py`(관측)+`deploy_all.py`(순수 판정)가 자기 세대 코드로 내고, 실행은 기존 배포기 호출뿐(사본 0). 스킬 action은 반드시 `deploy-skill.sh <skill> --release-approval`로 릴리스 ✅ 하나를 재사용한다(단독 핫픽스는 기존 per-skill 경로). `--plan` 판정 / `--verify` 전량 일치 시 영수증(`/srv/autophagy-private/deploy-all/receipt.json`) / `--apply` 배포기 실행→플러그인 변경 시 agent+peer 재시동→**전량 재판정이 하드 게이트**. 영수증은 clean 에만 서명되고(코드가 거부) `release_fully_deployed` 로컬 프로브가 현재 릴리스와 상시 대조 — 없음·불일치·판독불가 전부 FAIL. ⑤root·⑥RAG 는 상시 프로브 위임(`delegated` 필드). **미선언 관측(2026-09-03)**: 프로브는 선언된 행만 대조해 `~/.hermes/scripts` 의 손배포 사본을 보지 못했다 — 이제 매니페스트 계정마다 `scripts/`·`plugins/*/` 를 열거해 `HOME_DEPLOYED_PATTERN` 에 맞지만 선언되지 않은 경로를 `undeclared` 관측(계정·경로·sha 접두)으로 내고, 판정은 **경고**라 clean 영수증을 막지 않되 `--strict-undeclared` 면 drift 로 승격한다. 열거 실패는 여전히 관측 불가(exit 4), JSON 스키마는 키 추가만. **2026-09-05 PR #407**: `--apply`만 워크스테이션 `${DEPLOY_ALL_LOCK_DIR:-$HOME/.hermes/deploy-all}/apply.lock`을 flock으로 잡아 세션·완결 타이머 배포를 직렬화한다. 후행 실행은 `APPLY-LOCK-WAIT`→lock 획득 후 재판정→이미 전량 반영이면 `APPLY-LOCK-RECHECK`(`already fully deployed`)·영수증·rc 0으로 끝나고 배포기는 다시 부르지 않는다; `--plan`·`--verify`는 lock-free. 동시 실행 회귀는 PR #413이 release FIFO를 O_RDWR로 유지해 reader/writer 경합을 없앴다 |
| `release.sh` | script | `automation/release.sh` | 릴리스 버전 승인→서명 태그 컷(VA-1, **워크스테이션 전용**) — **`--bump {major,minor,patch}`(기본 patch, v1.1.0)** 로 버전 자리를 고르고, `release_plan.major_signals` 가 base..head 의 `POLICY_VERSION`·`SCHEMA_VERSION` 상수·`node_config._FIELD_NAMES`·interop 필수 키 변경을 찾으면 `--bump patch/minor` 를 `RELEASE-PLAN-BLOCK`(exit 4) 으로 거부하고 `--bump major` 의 패치노트에 `MAJOR: 운영자 조치 필요 —` 줄을 싣는다(승인 레코드 스키마 불변 — 산문만). `ensure_signed_tag` 는 같은 HEAD 에 다른 이름의 태그가 있으면 성공으로 끝내지 않고, prerelease 접미 태그는 최신 판정에서 무시한다; `release_version_for` 가 HEAD 에 이미 붙은 태그를 재사용해 **재실행이 재개**가 된다 — 완결기가 deploy 실패 뒤 next 를 다시 계산하면 그 이름 불일치 검사가 자기 태그를 거부한다(2026-09-03 v1.1.1 attempt 2 실측, PR #378)([소개](docs/기능소개/릴리스-버전-자리-선택.md)) — 새 본문은 사람이 이해할 bundle 이름·커밋 설명·전체 action hash를 우선한다. **renderer는 append-only 버전형**: 필드 없는 기존 레코드=v1 동결 문구, 신규=`render_version=2`; 게시된 문구를 같은 버전에서 바꾸면 과거 ✅가 무효가 되므로 금지. 승인은 노드 agent가 게시·판정하고, 다음 요청 전 `retire --head <latest-signed-head>`가 저장 HEAD 일치+Discord APPROVED를 재검증한 이전 레코드만 byte-exact 0600 `release-history/`로 원자 이동한다. 기본은 결정까지 무기한 대기해 ✅ 즉시 태그를 자른다. 낡은 pending 요청은 감사형 abandon 후 1회 재요청하며 결정 레코드는 불변이다. 전량 반영·영수증은 `deploy_all.sh --apply`가 완성한다. **2026-09-05 PR #406**: HEAD의 **살아 있는 승인 레코드 version을 next 계산보다 먼저 재사용**해 세션의 minor·major 선택과 완결기의 기본 patch가 서로 다른 태그를 만들지 못하게 한다 — 태그가 먼저 존재해야 했던 기존 재시도 가드보다 앞선 합의점이다. 과거 이중 태그는 삭제하지 않으며 그 과거 릴리스 반출의 `--version` 명시는 그대로다 |
| `peer_gateway_probe` | script | `automation/peer_gateway_probe.sh` | peer 승인 카드 수신 차단의 **읽기 전용 설정 드리프트 프로브**(2026-09-05, PR #409) — healthcheck LOCAL `peer_ignored_channels`가 peer `channel_directory.json`의 유일한 approvals id를 해석해 config 최상위 `discord.ignored_channels`와 대조한다. 있으면 PASS, 없으면 FAIL+`PEER-IGNORED-CHANNELS-RECOVERY`, 두 파일을 읽을 수 없으면 `PEER-IGNORED-CHANNELS-UNREADABLE`로 FAIL-closed한다. 자동 편집·재시동·승인 판정 변경은 없고 실행 중 게이트웨이의 설정 로드 증명도 아니다 — ops 판독에는 소유자의 파일별 `/usr/bin/cat --` sudoers 두 줄이 필요하다(셸·Python 권한 금지). [소개](docs/기능소개/peer-게이트웨이-승인채널-차단-프로브.md) **2026-09-06 PR #425**: 그 두 줄은 이제 설치기 자산 `automation/sudoers.d/autophagy-healthcheck-peer-read`(node config 로 계정·홈 렌더, root:root 0440)로 선언돼 새 노드 온보딩이 자동으로 받는다. 기존 노드는 `docs/guide/operations.md`「`peer_ignored_channels` 권한 복구」의 sudo 명령 한 번(그 파일만 `visudo -cf`·원자 교체·digest 대조)이 소유자 몫으로 남아 있다 — 코드 머지와 노드 설치는 별개이며 설치 전까지 ops 프로브는 여전히 UNREADABLE 이다 |
| `healthcheck_registry` | script | `automation/healthcheck_registry.sh` | `healthcheck.sh` 가 source 하는 **프로브 레지스트리**(2026-09-06, PR #421) — `LIVE_CHECKS` 배열(`표시 이름|프로브 종류|노드|계정|대상`)만 소유해 서비스 한 줄 추가가 진입점을 건드리지 않는다. 진입점 161·레지스트리 61 pure LOC, allowlist 매니페스트 출력은 분리 전과 바이트 동일, 프로브 디스패치 불변 |
| `pyrightconfig` | config | `pyrightconfig.json` | basedpyright **편집기 전용** 설정(2026-09-06, PR #420) — `skills/*/scripts` 19개·격리 `configs/stt-engines`·루트를 `executionEnvironments` 로 선언해 형제 import(`stt_gap`·`stt_split` 등)의 `reportImplicitRelativeImport` 를 없앤다(STT 2→0, mail 14→0, 기존 automation 진단 1건 불변). 런타임·테스트·CI 동작에 관여하지 않으며 `tests/unit/test_pyright_config.py` 가 선언 목록을 고정한다 |
...
| `release_applied_notice` | module | `automation/release_applied_notice.py` | 릴리스 **적용 완료** 소유자 DM 의 단일 구현(2026-09-05, t_0ec472d8·t_710c7d8b) — ✅ 는 허락이지 적용이 아니므로 완료를 셋의 교집합으로만 판정한다: 완결기 성공 마커 `completed/<sha>` · 노드 활성 포인터(`readlink <release_current>`) · 전량 재판정 영수증(RC-4)의 `release_sha`. 순수 `decide(ReleaseFacts)` 가 `Applied/Skip/Retry` 를 내고 순서가 곧 정책이다(SUPERSEDED → NO-TAG → VERSION-AMBIGUOUS → POINTER-UNREADABLE → POINTER-BEHIND → RECEIPT-MISSING → RECEIPT-STALE). **번호는 추정하지 않는다** — 그 sha 의 `v<major>.<minor>.<patch>` 태그 하나가 유일한 출처이고 둘이면 영구 보류다. 활성 포인터의 조상은 `SUPERSEDED` 로 기록만 한다(v1.2.3 사례). 워크스테이션 `sweep` 은 완결 단축회로 **앞**에서 매 틱 돌고 대상이 없으면 노드를 찌르지 않으며, 예외는 `RELEASE-APPLIED-SWEEP-FAIL` 한 줄로 끝나 완결 종료코드를 바꾸지 않는다. 발신은 `release_approval_remote.sh` 에 `RELEASE_APPROVAL_MODULE` 로 실려 노드 agent 자격으로 돌고, 거기서 포인터를 한 번 더 확인한 뒤(`APPLIED-POINTER-MISMATCH` exit 4) `owner_notice.notify_owner_dm` 에 넘긴다. 멱등은 `notified/<sha>`·`notify-skipped/<sha>`, 전송 실패는 마커 없이 다음 틱 재시도(상한 없음)([소개](docs/기능소개/릴리스-적용-완료-DM-통지.md)) |
| `notify_owner_dm` | fn | `automation/owner_notice.py` | 소유자가 **DM 을 명시한** 통지 전용(2026-09-05) — `notify_owner` 와 달리 소유자 통지 채널이 설정돼 있어도 그리로 새지 않고 항상 소유자 DM 으로 간다(릴리스 적용 완료가 첫 소비자). DM 오픈은 여전히 이 파사드 안에서만 일어나고(ON-2/ON-3), 미설정은 `NOTIFY-UNCONFIGURED`, 실패는 `NOTIFY-FAILED: <type>` 를 남기며 **예외는 절대 던지지 않는다** |
| `release_notes` | module | `automation/release_notes.py` | 릴리스 변경 상세 — 경로→배포 번들 판정(`BundleMap`, `build_plan` 의 표면 분류와 **같은 한 벌**), `git log --name-only` 파싱(`parse_commits`), 번들별 절(`### <번들>`·`### 여러 번들`·`### 공통·기타`)로 묶은 본문, 그리고 `post_details` 의 순차 게시(실패 시 `RELEASE-DETAIL-POST-FAIL <type> posted=i/n`). 원문 항목은 절대 줄이지 않는다 — 한도는 `release_spec.split_messages` 가 줄 경계로 나눠서 지킨다([소개](docs/기능소개/릴리스-승인-메시지-상세-분할.md)) |
| `ReleaseSpec.render` v3 | fn | `automation/release_spec.py` | 승인 카드 v3(신규 기본): 버전·배포 기준·번들 이름·`action_hash`·선택적 `MAJOR:` 줄·"변경 상세: 아래 n개 메시지"·승인 방법만 싣고 패치 원문은 싣지 않는다(1900자 초과는 fail-closed). `split_messages`/`detail_header` 가 같은 모듈에 있는 이유는 노드 게이트가 **레코드만으로** 카드의 n 과 상세 본문을 재생해야 하고 staging 집합에 계획·git 모듈이 들어가면 `deploy-skill.sh` 가 STAGE-BLOCK 으로 서기 때문. 저장된 `render_version` 1·2 는 바이트 동결(회귀 고정) |
...
- `configs/stt-engines/`는 pyannote·WhisperX를 별도 venv에 가둔 서드파티 격리 예외다. `uv.lock` 고정 설치·모델 캐시는 체크아웃 밖이며, CLI 계약은 해당 README가 소유한다. 소스 공개와 비공개 원음·평가 상태 보관은 별개다.
...
- ops·dashboard 등 다른 unit은 이 규칙 대상이 아니다 — Hermes gateway(agent·peer) 세트에만 적용.
...
- 승인 본문에는 version·HEAD, **변경 표면 이름**, 사람이 읽는 변경 설명, 전체
  ...
  없다.
- **Discord 한도를 넘는 변경 설명은 줄이지 않고 나눈다**(2026-09-05 개정, 이전 규칙인
  "완전한 줄 단위로 줄이고 생략 줄 수를 명시한다"는 폐기). 승인 카드(`render_version=3`)는
  버전·배포 기준·번들 이름·`action_hash`·뒤따르는 상세 메시지 수만 싣고, 커밋 원문은
  번들별 절(`### <번들>`)로 묶은 **「변경 상세」 메시지들**이 싣는다
  (`automation/release_notes.py`). 상세 메시지는 저마다 같은 릴리스 번호와 배포 기준을
  머리글로 달아 어느 승인에 속하는지 스스로 말하고, 번들 목록·승인 문구는 카드에만 한
  번 나온다. 렌더러는 `+N건`·`omitted`·말줄임표를 만들지 않으며 한 줄이 한도보다 길면
  버리는 대신 이어 붙일 수 있게 나눈다 — 중요 변경이 생략 구간에 숨는 경로 자체가
  없다. 그래도 카드 본문이 1900자를 넘으면(번들 수가 과도한 릴리스) 게시하지 않고
  막는다.
...
- **선행 충돌 주의**: `.omo/plans/repair-report-{core,rollout}.md`(타 세션, 미추적 상태)가 `repair_ops_core.py`·`automation/interop/report.py`·**`automation/repair/systemd/*`(예외 없음)**·일부 승인 흐름의 변경을 금지하고 있다. 위 롤아웃 ②는 systemd 유닛을 건드리므로 **해당 계획과 순서를 조율한 뒤** 진행한다. 먼저 손대면 다른 세션의 확정 설계를 덮어쓰게 된다(2026-07-21 선례).
...
## 용어 교정 위치 규칙 (cha 지시, 2026-09-05)

**용어 교정은 음성→전사본 단계가 아니라 전사본→산출 문서(회의록·lifelog 노트·보고서 …) 단계에서 한다 — 문서 종류마다 중첩된 교정 참고 문서를 쓰고, 문서에 실린 원문 인용 구역은 고치지 않는다.**

- **왜 전사본에서 고치지 않는가**: 전사본은 증거다. 치환을 거기에 새기면 원래 낱말이 어디에도 남지 않는다 — 근접 교정은 실제로 틀린 적이 있다(소유자 용어집의 「성금=선금」이 이 과제의 핵심어 「기성금」을 「기선금」으로 깨뜨렸다). 문서 단계에서 고치면 잘못된 교정은 참고 문서를 고쳐 그 문서를 다시 만들면 회복된다. 그리고 문서마다 어휘가 다르다 — 회의록은 기관명, 라이프로그는 사람·장소, 제안서는 과제 용어다. 하나의 평면 용어집을 전사본에 바르면 그 차이를 표현할 자리가 없다.
- **층은 문서 종류를 따라 중첩한다**: `autophagy/용어집.csv`(모든 산출물) → `autophagy/<문서 종류>/용어집.csv` → `autophagy/<문서 종류>/<과제>/용어집.csv`. 깊은 층이 같은 낱말을 덮어쓰고 나머지는 바깥 층이 그대로 준다. 폴더 이름의 정본은 `automation/drive_taxonomy.CATEGORIES` 이고, Drive 산출물이 아닌 문서 종류만 `term_glossary.EXTRA_FOLDERS` 가 채운다.
- **단일 정의**: 엔진 `automation/term_correction.py`(파싱·지목 치환·자모 근접 교정) · 층 해석 `automation/term_glossary.py` · 감사 로그 `automation/term_correction_log.py` 세 개뿐이다. 스킬은 위임만 하고 사본을 만들지 않는다 — 갈라지면 같은 낱말이 문서마다 달라진다.
- **새 스킬이 지켜야 할 것**: ① 렌더 **전** 해석 결과(요약·결정·액션·사람·장소)에 교정을 걸고, ② 문서에 실리는 원문 인용 구역(회의록 부록 `### C. 원문 전사본`, lifelog `## 전문`, 메일 회신의 `-----원본 메시지-----` 아래)은 바이트 그대로 둔다. ③ 교정 내역은 `term_correction_log.record(document=…, stage=…)` 로 **어절만** 남긴다(문맥을 남기면 본문이 로그로 샌다). ④ Drive 조회는 `DRIVE_PUBLISH_ENABLED` 옵트인이고 실패는 노드 캐시로 내려가며, 교정 실패는 문서 생성을 절대 막지 않는다(fail-soft). 교정은 로컬 계산이라 모델을 부르지 않으므로 민감도 게이트 대상이 아니다.
- **전사 프롬프트 힌트는 교정이 아니다**: 바른 용어를 인식 **전에** 조건으로 주는 것은 남는다(전사본을 더 옳게 만든다). 이 규칙이 금지하는 것은 **인식된 전사본 본문을 사후에 치환**하는 일이다.
- **강제**: `tests/unit/test_term_correction_conformance.py` 가 산출 문서 생성 경로 인벤토리를 들고 (a) 교정 엔진의 사본이 `automation/` 밖에 있으면 RED, (b) 전사본 생성 경로가 교정을 부르면 RED, (c) 채택하지 않은 문서 종류는 `_EXEMPT` 에 사유와 함께 등록되어야 한다. 산문이 아니라 코드가 진실이다.
- 절차·형식·환경변수·실패 마커의 단독 정본은 [docs/guide/용어-교정-규약.md](docs/guide/용어-교정-규약.md) 다. 이 절은 불변식만 들고 있고 절차를 복사하지 않는다.
- **배경**: 2026-09-05 소유자 판단 — "① 음성→전사본에서 교정할지 ② 전사본→회의록·lifelog 에서 교정할지 고민했는데, ②에서 문서별 중첩 교정 참고 문서를 쓰는 게 좋겠다". 직전 사이클(PR #390/#391)이 ①로 구현돼 있었으므로 그 위치를 옮긴다.
