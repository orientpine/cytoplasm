...
| `profiles` | module | `automation/install/profiles.py` | 프로필→헬스체크 선언의 단일 정의. 설치기가 `/etc/autophagy/healthcheck.env`(root:root 0644)를 쓰고 registry는 환경변수가 unset일 때만 파일로 fallback한다. 생략은 기존 계획 유지, `--suggest` 는 복구용(기존 노드·수동 서비스 변경 뒤)이다 |
